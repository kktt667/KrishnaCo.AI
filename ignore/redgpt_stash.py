    ...
    def run(self):
        if not st.session_state.authenticated:
            ...
            if st.session_state.current_chat_id:
                ...
                # Chat title and model info
                st.title(current_chat['title'])
                st.caption(f"Using {current_chat['model']}")
                ...
                # Handle input when Enter is pressed and input has changed
                if user_input and user_input != st.session_state.previous_input:
                    with st.spinner("Generating response..."):
                        ...
                        # Rerun to clear input (the text input will be empty on rerun)
                        st.rerun()

            else:
                st.title("Welcome to AI Chat")
                st.write("Create a new chat or select an existing one to begin.")
